...
不過 中間的過程 有點讓人分心 換句話說 就是

$$
\delta (D \cdot E) = \delta (D) \cdot E + D \cdot \delta (E)
$$

$$
= \delta (D) \cdot E + \varepsilon E \cdot \delta (E)
$$

$$
= \delta (D) \cdot E + E \cdot \delta (\varepsilon E)
$$

$$
= \delta (D) \cdot E + E \cdot \delta (D) = 2 \delta (D) \cdot E 
$$

整個脈絡就是 增加一點電荷 最後 整個積分增加的能量

推導過程將 電荷增量 推至 場的增量

而我指出的關鍵就是 

最後 可以把 場的增量 轉換成 整個積分增量的關鍵

在這裡 介電質必須是線性的 才能把 1/2 給推導出來
